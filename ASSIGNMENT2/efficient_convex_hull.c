...
// return value: 선들의 집합(lines)에 대한 포인터
t_line *upper_hull( t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity){
	//점의 집합이 공집합일때, p1, pn을 line segment에 추가
	if(num_point == 0){
	...
	float a = p1.y- pn.y;
	float b = pn.x - p1.x;
	float c = (pn.x-p1.x)*pn.y - (pn.y-p1.y)*pn.x;
	float maxDis = 0;
	for(int i=0; i<num_point;i++){
		if(distance(a,b,c,points[i])>maxDis){
			maxDis = distance(a,b,c,points[i]);
			pmax = points[i];
		}
	}
...
void separate_points( t_point *points, int num_point, t_point from, t_point to, t_point *s1, t_point *s2, int *n1, int *n2){
	...
	*n2 = 0;
	for(int i=0; i<num_point; i++){
		if((from.x == points[i].x && from.y == points[i].y) || (to.x  == points[i].x && to.y == points[i].y)) continue;
		float res = (from.y-to.y)*points[i].x - (from.x-to.x)*points[i].y + (from.x-to.x)*from.y - (from.y-to.y)*from.x;
		if(res < 0){
			s1[*n1] = points[i];
			*n1 += 1;
		}
